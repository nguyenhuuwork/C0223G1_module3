create database furama_resort_again;
use furama_resort_again;
create table vi_tri(
...
insert into furama_resort_again.vi_tri (ten_vi_tri) value ('Quản Lý');
insert into furama_resort_again.vi_tri (ten_vi_tri) value ('Nhân Viên');
insert into furama_resort_again.bo_phan (ten_bo_phan) value ('Sale-Marketing');
insert into furama_resort_again.bo_phan (ten_bo_phan) value ('Hành chính');
insert into furama_resort_again.bo_phan (ten_bo_phan) value ('Phục vụ');
insert into furama_resort_again.bo_phan (ten_bo_phan) value ('Quản lý');
insert into furama_resort_again.trinh_do (ten_trinh_do) value ('Trung Cấp');
insert into furama_resort_again.trinh_do (ten_trinh_do) value ('Cao Đẳng');
insert into furama_resort_again.trinh_do (ten_trinh_do) value ('Đại Học');
insert into furama_resort_again.trinh_do (ten_trinh_do) value ('Sau Đại Học');
insert into furama_resort_again.nhan_vien (ho_ten,ngay_sinh,so_cmnd,luong,so_dien_thoai,email,dia_chi,ma_vi_tri,ma_trinh_do,ma_bo_phan) 
values ('Nguyễn Văn An', '1970-11-07','456231786',10000000,'0901234121','annguyen@gmail.com','295 Nguyễn Tất Thành, Đà Nẵng',1,3,1),
 ...
 insert into furama_resort_again.loai_khach (ten_loai_khach) 
 values('Diamond'),('Platinium'),('Gold'),('Silver'),('Member');
 insert into furama_resort_again.khach_hang (ho_ten,ngay_sinh,gioi_tinh,so_cmnd,so_dien_thoai,email,dia_chi,ma_loai_khach)
...
insert into furama_resort_again.kieu_thue (ten_kieu_thue)
values ('year'),('month'),('day'),('hour');
insert into furama_resort_again.loai_dich_vu (ten_loai_dich_vu)
values ('Villa'),('House'),('Room');
insert into furama_resort_again.dich_vu (ten_dich_vu,dien_tich,chi_phi_thue,so_nguoi_toi_da,tieu_chuan_phong,mo_ta_tien_nghi_khac,dien_tich_ho_boi,so_tang,ma_kieu_thue,ma_loai_dich_vu)
...
insert into furama_resort_again.dich_vu_di_kem (ten_dich_vu_di_kem, gia, don_vi, trang_thai)
...
insert into furama_resort_again.hop_dong (ngay_lam_hop_dong, ngay_ket_thuc, tien_dat_coc, ma_nhan_vien, ma_khach_hang, ma_dich_vu)
...
insert into furama_resort_again.hop_dong_chi_tiet (so_luong, ma_hop_dong, ma_dich_vu_di_kem)
values (5,2,4),
(8,2,5),
(15,2,6),
(1,3,1),
...
select*
from khach_hang kh
where (timestampdiff(year,kh.ngay_sinh,curtime()) between 18 and 50) and 
dia_chi like '%Đà Nẵng' or
dia_chi like '%Quảng Tri';

-- 4.	Đếm xem tương ứng với mỗi khách hàng đã từng đặt phòng bao nhiêu lần.
-- Kết quả hiển thị được sắp xếp tăng dần theo số lần đặt phòng của khách hàng. 
-- Chỉ đếm những khách hàng nào có Tên loại khách hàng là “Diamond”.

select kh.ho_ten, kh.ma_khach_hang, count(*)
from khach_hang kh
join loai_khach lk
...
select lk.ten_loai_khach, kh.ma_khach_hang, kh.ho_ten, hd.ma_hop_dong, hd.ngay_lam_hop_dong, hd.ngay_ket_thuc, ifnull(sum(chi_phi_thue + ifnull((so_luong*gia),0)),0)
from loai_khach lk
join khach_hang kh
...
select dv.ma_dich_vu, dv.ten_dich_vu, dv.so_nguoi_toi_da, dv.chi_phi_thue, ldv.ten_loai_dich_vu
from dich_vu dv 
join loai_dich_vu ldv
on dv.ma_loai_dich_vu = ldv.ma_loai_dich_vu
join hop_dong hd
...
select month(ngay_lam_hop_dong) , count(hd.ma_hop_dong)
from khach_hang kh
join hop_dong hd
...
join hop_dong hd
on dv.ma_dich_vu = hd.ma_dich_vu
where ( quarter(ngay_lam_hop_dong) between 1 and 2) and year(ngay_lam_hop_dong) ='2021'
);
...
select hd.ma_hop_dong, ldv.ten_loai_dich_vu, dvdk.ten_dich_vu_di_kem, ldv.ma_loai_dich_vu, hdct.ma_hop_dong_chi_tiet,count(hdct.so_luong)
from hop_dong hd
join dich_vu dv
on hd.ma_dich_vu = dv.ma_dich_vu
join loai_dich_vu ldv
...
group by hd.ma_hop_dong, ldv.ten_loai_dich_vu, dvdk.ten_dich_vu_di_kem, ldv.ma_loai_dich_vu, hdct.ma_hop_dong_chi_tiet;
...
select nv.ma_nhan_vien, nv.ho_ten, nv.so_dien_thoai, nv.dia_chi, td.ten_trinh_do, bp.ten_bo_phan, count(hd.ma_hop_dong)
from nhan_vien nv
join trinh_do td
...
left join hop_dong hd
on nv.ma_nhan_vien = hd.ma_nhan_vien
where hd.ma_hop_dong is null and nv.ma_nhan_vien not in (
...
where nv.ma_nhan_vien in( 1,4,5,6,8,9);
